...
def test(data,
    ...
    # Dataloader
    if not training:
        iouv = torch.linspace(opt.ap_thres, 0.95, 10).to(device)  # iou vector for mAP@0.5:0.95 zjq
        niou = iouv.numel()
    ...
    for batch_i, (img, ir, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)): #zjq
        ...
        for si, pred in enumerate(out):
            ...
            if save_txt: 
                ...
                if opt.data.endswith('DOTA.yaml'):
                    for *xyxy, conf, cls in predn.tolist():
                        line = (path.stem, conf, *xyxy) if save_conf else (path.stem,*xyxy) # label format
                        with open(save_dir / 'labels' / ('Task2_'+ names[cls] + '.txt'), 'a+') as f:
                            f.write(line[0] + ' ' + ('%g ' * (len(line)-1)).rstrip() % line[1:] + '\n')
                else:
                    for *xyxy, conf, cls in predn.tolist():
    ...
    print(pf % ('all', seen, nt.sum(), mp*100, mr*100, map50*100, map*100))
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            print(pf % (names[c], seen, nt[c], p[i]*100, r[i]*100, ap50[i]*100, ap[i]*100))
            # with open("trying.txt", 'a+') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/use/exp10_8bs_100e_nwpu/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/NWPU.yaml', help='*.data path')
    parser.add_argument('--batch-size', type=int, default=4, help='size of each image batch')
    parser.add_argument('--img-size', type=int, default=512, help='inference size (pixels)')
    parser.add_argument('--input_mode', type=str, default='RGB') #RGB IR RGB+IR RGB+IR+fusion
    parser.add_argument('--ap-thres', type=float, default=0.5, help='object confidence threshold')
    ...
    parser.add_argument('--plots', action='store_true', help='save visible results')
