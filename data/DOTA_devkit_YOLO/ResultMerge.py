...
def mergebypoly(srcpath, dstpath):
    ...
    mergebase(srcpath,
              ...
              py_cpu_nms_poly)
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='resultmerge.py')
    parser.add_argument('--scratch', type=str, default='runs/test/exp16/labels/', help='model.pt path(s)')
    opt = parser.parse_args()
    # see demo for example
    # mergebypoly(r'path_to_configure', r'path_to_configure')
    # mergebyrec()
    srcpath = opt.scratch
    dstpath = srcpath.replace('labels','labels_merge')
    if not os.path.exists(dstpath):
        os.makedirs(dstpath)
    mergebyrec(srcpath, dstpath)
